...
const AdminManageRestaurant = () => {
  ...
  const handleSaveMenuItem = async (item) => {
    try {
      await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/restaurants/${id}/menu/${item._id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleDeleteItem = async (itemId) => {
    try {
      await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/restaurants/${id}/menu/${itemId}`,
        {
          method: "DELETE",
  ...
  const handleAddMenuItem = async () => {
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/restaurants/${id}/menu`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
        ...
        {menuItems.map((item, index) => (
          ...
          >
            <input
              value={item.name}
              onChange={(e) =>
                handleMenuUpdate(index, "name", e.target.value)
              }
              className="border p-2 rounded"
            ...
            <input
              value={item.image}
              onChange={(e) =>
                handleMenuUpdate(index, "image", e.target.value)
              }
              className="border p-2 rounded"
              placeholder="Image URL"
            />
            <input
              value={item.category}
              onChange={(e) =>
                handleMenuUpdate(index, "category", e.target.value)
              }
              className="border p-2 rounded"
            ...
            <input
              ...
              type="number"
              onChange={(e) =>
                handleMenuUpdate(index, "price", e.target.value)
              }
              className="border p-2 rounded"
              placeholder="Price"
            />
